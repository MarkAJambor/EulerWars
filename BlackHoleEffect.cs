...
public class BlackHoleEffect : MonoBehaviour
{
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        if (shader && material)
        {
            ...
            Vector2 pos = new Vector2(this.camera.WorldToScreenPoint(BH.transform.position).x / this.camera.pixelWidth, this.camera.WorldToScreenPoint(BH.transform.position).y / this.camera.pixelHeight);
